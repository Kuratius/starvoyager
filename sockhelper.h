...
class sockhelper //Class that helps with socket operations, giving them much easier non-blocking semantics
{
	...
	uint32_t getcount(); //Get the socket throughput byte count, and reset it to zero

	private:
	static uint32_t alarmcallback(uint32_t dly,void* from); //Callback handling the alarm when pumping blocks for too long
	...
	uint32_t cnt; //Count of bytes come through the socket
